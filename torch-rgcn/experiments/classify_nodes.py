...
def train_model(dataset,
    ...
    model_name = f"{dataset['name']}_{rgcn['model']}_h{nhid}_l{nlayers}"
    if decomposition is not None:
        model_name += f"_{decomposition['type']}"
    ...
    install_hooks()
    # Disable recording during training, only enable during evaluation
    recorder.disable()
    ...
    # Enable GEMM recording for evaluation
    recorder.enable()
    model.eval()
    classes = model()[test_idx, :].argmax(dim=-1)
    test_accuracy = accuracy_score(classes.cpu(), test_lbl.cpu()) * 100  # Note: Accuracy is always computed on CPU
