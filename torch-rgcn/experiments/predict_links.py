import sys
sys.path.append('/home/nizhj/gnn_sparsity/torch-rgcn')
...
from utils.gemm_hooks import init_recorder, install_hooks, uninstall_hooks, finalize_recorder
...
def train(dataset,
    ...
    encoder_name = encoder.get('model', 'rgcn')
    decoder_name = decoder.get('decoder', 'distmult')
    nhid = encoder.get('hidden_size', 'default')
    nlayers = encoder.get('num_layers', 2)
    model_name = f"{dataset['name']}_{encoder_name}_{decoder_name}_h{nhid}_l{nlayers}"
    if 'decomposition' in encoder and encoder['decomposition']:
        model_name += f"_{encoder['decomposition']['type']}"
    
    # Setup directories
    base_dir = Path('/home/nizhj/gnn_sparsity/torch-rgcn')
    ckpt_dir = base_dir / 'ckpt' / model_name
    output_dir = base_dir / 'output'
    ckpt_dir.mkdir(parents=True, exist_ok=True)
    
    # Initialize GEMM recorder
    recorder = init_recorder(str(output_dir), model_name)
    install_hooks()
    # Disable recording during training, only enable during final evaluation
    recorder.disable()
    ...
    with torch.no_grad():
        ...
        # Enable GEMM recording for final evaluation
        recorder.enable()
        model.eval()
        ...
        # Disable recording after evaluation
        recorder.disable()
    
    # Finalize GEMM recording
    finalize_recorder()
    uninstall_hooks()
    print(f'GEMM recordings saved to {output_dir / model_name}')
