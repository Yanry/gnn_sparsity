...
def sum_sparse(indices, values, size, row_normalisation=True, device='cpu'):
    ...
    # Use modern torch.sparse_coo_tensor API instead of deprecated constructors
    values = torch.sparse_coo_tensor(indices.t(), values, torch.Size(size), device=device)
